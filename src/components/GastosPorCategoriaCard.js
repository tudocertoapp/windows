...
export function GastosPorCategoriaCard({ catBreakdown, totalExpense, formatCurrency, mask, colors, title = 'Gastos por categoria' }) {
          ...
          <View key={cat} style={s.catItem}>
            <View style={s.catHeader}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <View style={[s.catDot, { backgroundColor: CATEGORY_COLORS[cat] || '#6b7280' }]} />
                <Text style={[s.catName, { color: colors.text }]}>{cat}</Text>
              </View>
              <Text style={[s.catAmount, { color: colors.text }]}>{m(fmt(amount))}</Text>
            </View>
            <View style={[s.progressBar, { backgroundColor: colors.border }]}>
              <View style={[s.progressFill, { width: `${pct}%`, backgroundColor: CATEGORY_COLORS[cat] || '#6b7280' }]} />
            </View>
          </View>
        );
      })}
